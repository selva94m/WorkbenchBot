...
let error_resposne = "Sorry I couldn't understand What you said. Please check your internet connection and try again";
...
function makeResponse(txt_dialog_utterance, callBackFunction=()=>{}){
    ...
    response.then((value)=>{
            let response_utterance = DialogUtterance(value['result']['fulfillment']['speech'], 'simple-bot-'+value['id'], new Date(value['timestamp']));
            callBackFunction(response_utterance);
    }).catch((error)=>{
        console.log(error);
        let response_utterance = DialogUtterance(error_resposne, 'simple-bot-error');
        callBackFunction(response_utterance);
    });
}
